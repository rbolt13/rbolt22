...
This analysis will uses packages the [tidyverse](https://www.tidyverse.org/) to wrangle the data, and plotly to make interactive graphs.
...
When we consider the use case above, and that a canvas costs about `$`3. Then a budget conscious user can assume their **budget for paint canvas** will be about **`$`40 per season**, and about **`$`1,210 for all 403 episodes**.
...
When considering out use case, a budget conscious viewer may consider starting their journey at season 1 where the least amounf ot colors are used. 
